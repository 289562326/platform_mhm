	...
	$.validationEngineLanguage = {
		newLang:function(){
			$.validationEngineLanguage.allRules = {
				...
				"telPhone":{
					//"regex":/^1(3|4|5|7|8)\d{9}$/,//13123456789
					//"regex":/^((0\d{2,3})-)(\d{7,8})(-(\d{3,}))?$/,//0755-88888888
					//(1)电话号码由数字、"("、")"和"-"构成 
					//(2)电话号码为3到8位 
					...
					"regex":/(^[0-9]{3,4}\-[0-9]{3,8}$)|(^[0-9]{3,8}$)|(^\([0-9]{3,4}\)[0-9]{3,8}$)|(^0{0,1}13[0-9]{9}$)|(^0{0,1}14[0-9]{9}$)|(^0{0,1}15[0-9]{9}$)|(^0{0,1}17[0-9]{9}$)|(^0{0,1}18[0-9]{9}$)/,
					"alertText":"* 无效的电话号码"
				},
				"email":{
				 ...
				 * [a-z0-9A-Z]:表示可以出现任意一个英文字母或者数字 
				 * [\u4e00-\u9fa5]汉字的 
				 * [,\.;\:"'!] 可以匹配 ,.;:"'! 标点符号，估计只能写一些常用的了（正则表达式的关键字要用 \ 进行转义）
				 * 
				 */
				"noCustom":{
					"regex":/^[^<>& '%|]+$/,
					"alertText":"* 不允许使用特殊字符<>&'%|和空格"
				},
				"noSpecific":{
					"regex":/^[^:*"?<>& '%|\\\/]+$/,
					"alertText":"* 不允许使用特殊字符:*\"?<>&'%|\\\/和空格"
				...
				"maxFileSize": { 
					"func": function(field,rules,i,options){
						...
						var filemaxsize = 1024 * 1024 * 20; //20M

						if (isIE && !target.files) {
							var filePath = target.value;
							var fileSystem = new ActiveXObject("Scripting.FileSystemObject");
							if (!fileSystem.FileExists(filePath)) {
								return "File is not exsist！";
							}
